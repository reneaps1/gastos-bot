...
function estadoMargen(ingreso: number | null, margen: number | null) {
  if (ingreso == null || margen == null) {
  ...
  if (ingreso > 0 && margen / ingreso < 0.1) {
...
function crearEscenario(q: ForecastQuincena): ScenarioDraft {
  return {
...
function ForecastCard({ q, scenario }: { q: ForecastQuincena; scenario: ScenarioResult | null }) {
  const estado = scenario ? estadoMargen(scenario.ingreso, scenario.margen) : estadoMargen(q.ingresoEsperado, q.margenEstimado)
  const necesidad = scenario?.necesidad ?? q.necesidadEstimada

  return (
    <article className={`rounded-2xl border bg-white dark:bg-slate-800 overflow-hidden ${scenario ? 'border-amber-300 dark:border-amber-700 ring-1 ring-amber-200 dark:ring-amber-800/50' : 'border-slate-200 dark:border-slate-700'}`}>
      <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {scenario && <span className="text-[9px] font-bold uppercase tracking-wide text-amber-700 dark:text-amber-300">escenario</span>}
          </div>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">{formatDate(q.fechaInicio)} – {formatDate(q.fechaFin)}</p>
        </div>
        <div className="text-right">
          <p className="text-[10px] text-slate-400 dark:text-slate-500">{scenario ? 'Necesidad escenario' : 'Necesidad'}</p>
          <p className="font-bold text-slate-800 dark:text-slate-100 tabular-nums">{formatMXN(necesidad)}</p>
        </div>
      </div>
      ...
      <div className="p-4 space-y-2 text-xs">
        ...
        {scenario && (
          <div className="rounded-lg bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/60 px-3 py-2 mt-2">
            <div className="flex justify-between gap-4 text-amber-800 dark:text-amber-200 font-semibold"><span>Escenario</span><span className="tabular-nums">{formatMXN(scenario.necesidad)}</span></div>
            <div className="flex justify-between gap-4 text-[10px] text-amber-700 dark:text-amber-300 mt-1"><span>Impacto vs forecast</span><span className="tabular-nums">{scenario.diferenciaVsForecast > 0 ? '+' : ''}{formatMXN(scenario.diferenciaVsForecast)}</span></div>
          </div>
        )}
...
export function PresupuestoForecast({ today, presupuestos }: Props) {
  ...
  const lineasScenario = useMemo(() => {
    if (!scenarioQ) return []
    return presupuestos.filter(p => p.quincenaId === scenarioQ.quincenaId && p.categoria.tipo === 'Gasto' && p.estadoLinea !== 'Cancelada')
  ...
  const scenarioResult = useMemo<ScenarioResult | null>(() => {
    if (!scenario || !scenarioQ) return null

    const ingreso = scenario.ingreso.trim() === ''
      ? scenarioQ.ingresoEsperado
      : amount(scenario.ingreso, scenarioQ.ingresoEsperado ?? 0)
    let fijo = amount(scenario.fijo, scenarioQ.gastoFijo)
    let variable = amount(scenario.variable, scenarioQ.gastoVariableEstimado)
    const creditos = amount(scenario.creditos, scenarioQ.creditosProgramadosExtra)
    const ahorro = amount(scenario.ahorro, scenarioQ.ahorroPlaneado)
    const extraordinario = amount(scenario.extraordinario, 0)

    let ajusteLinea = 0
    if (lineaSeleccionada && scenario.lineaMonto.trim() !== '') {
      const baseLinea = Number(lineaSeleccionada.montoEfectivo)
      const nuevoMonto = amount(scenario.lineaMonto, baseLinea)
      ajusteLinea = nuevoMonto - baseLinea
      if (lineaSeleccionada.clasificacion === 'Fijo') fijo = Math.max(0, fijo + ajusteLinea)
      else variable = Math.max(0, variable + ajusteLinea)
    }

    const necesidad = fijo + variable + creditos + extraordinario + ahorro
  ...
  }, [scenario, scenarioQ, lineaSeleccionada])

  function actualizar<K extends keyof ScenarioDraft>(key: K, value: ScenarioDraft[K]) {
    setScenario(prev => prev ? { ...prev, [key]: value } : prev)
  }

  function cambiarQ(id: string) {
    const q = qs.find(item => item.quincenaId.toString() === id)
    if (q) setScenario(crearEscenario(q))
  }

  function seleccionarLinea(id: string) {
    const linea = lineasScenario.find(p => p.id.toString() === id)
  ...
  if (loading) {
    return (
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5">
        <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-1.5"><CalendarRange size={15} className="text-violet-500" /> Forecast financiero</p>
  ...
  const totalMargen = qs.filter(q => q.margenEstimado != null).reduce((s, q) => s + (q.margenEstimado ?? 0), 0)
  const todasConIngreso = qs.length > 0 && qs.every(q => q.ingresoEsperado != null)
  const comparacion = scenarioQ && scenarioResult ? [
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-1.5"><CalendarRange size={15} className="text-violet-500 dark:text-violet-400" /> Forecast financiero</p>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">Próximas {qs.length || 3} Q completas · fijos conocidos + comportamiento variable + pagos de crédito + ahorro planeado.</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          {primera && (
            <button
              type="button"
              onClick={() => setScenario(prev => prev ? null : crearEscenario(primera))}
      ...
      {qs.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div className={`rounded-xl p-4 ${todasConIngreso && totalMargen < 0 ? 'bg-rose-50 dark:bg-rose-950/40 dark:ring-1 dark:ring-rose-800/50' : 'bg-sky-50 dark:bg-sky-950/40 dark:ring-1 dark:ring-sky-800/50'}`}>
              <p className={`text-xs mb-1 ${todasConIngreso && totalMargen < 0 ? 'text-rose-600 dark:text-rose-400' : 'text-sky-600 dark:text-sky-400'}`}>Margen proyectado</p>
              <p className={`text-xl font-bold tabular-nums ${todasConIngreso && totalMargen < 0 ? 'text-rose-700 dark:text-rose-300' : 'text-sky-700 dark:text-sky-300'}`}>{todasConIngreso ? formatMXN(totalMargen) : '—'}</p>
          ...
          {scenario && scenarioQ && scenarioResult && (
            <div className="rounded-2xl border-2 border-amber-300 dark:border-amber-700 bg-amber-50/60 dark:bg-amber-950/20 overflow-hidden">
              <div className="p-4 md:p-5 border-b border-amber-200 dark:border-amber-800/60 flex items-start justify-between gap-3 flex-wrap">
                ...
                <button type="button" onClick={() => setScenario(crearEscenario(scenarioQ))} className="inline-flex items-center gap-1.5 text-xs font-medium text-amber-700 dark:text-amber-300 hover:underline cursor-pointer"><RefreshCw size={12} /> Restablecer Q</button>
              </div>
                ...
                <div className="xl:col-span-3 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    <label className="block">
                      ...
                      <select value={scenario.quincenaId} onChange={e => cambiarQ(e.target.value)} className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-400">
                        {qs.map(q => <option key={q.quincenaId} value={q.quincenaId}>{q.codigo}</option>)}
                      </select>
                    </label>
                    <ScenarioField label="Ingreso esperado" value={scenario.ingreso} onChange={v => actualizar('ingreso', v)} hint="Vacío conserva 'sin dato' si no hay ingreso base." />
                    <ScenarioField label="Ahorro" value={scenario.ahorro} onChange={v => actualizar('ahorro', v)} />
                    <ScenarioField label="Gasto fijo" value={scenario.fijo} onChange={v => actualizar('fijo', v)} />
                    <ScenarioField label="Gasto variable" value={scenario.variable} onChange={v => actualizar('variable', v)} hint={`Forecast base: ${formatMXN(scenarioQ.gastoVariableEstimado)}`} />
                    <ScenarioField label="Créditos extra" value={scenario.creditos} onChange={v => actualizar('creditos', v)} />
                    <ScenarioField label="Compromiso extraordinario" value={scenario.extraordinario} onChange={v => actualizar('extraordinario', v)} hint="Gasto hipotético que aún no existe en Milo." />
                  </div>
                  ...
                  <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-900/40 p-4">
                    <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">Opcional. La diferencia se suma al fijo o variable según la clasificación de la línea.</p>
                      </div>
                      {scenarioResult.ajusteLinea !== 0 && <span className={`text-xs font-semibold ${scenarioResult.ajusteLinea > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>{scenarioResult.ajusteLinea > 0 ? '+' : ''}{formatMXN(scenarioResult.ajusteLinea)}</span>}
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <label className="block">
                        <span className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">Línea</span>
                        <select value={scenario.lineaId} onChange={e => seleccionarLinea(e.target.value)} className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-amber-400">
                          <option value="">Sin ajuste por línea</option>
                          {lineasScenario.map(p => <option key={p.id} value={p.id}>{p.categoria.nombre} · {p.descripcion} ({formatMXN(Number(p.montoEfectivo))})</option>)}
                        </select>
                      </label>
                      <label className="block">
                        <span className="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">Nuevo monto de la línea</span>
                        <input type="number" min="0" step="0.01" disabled={!lineaSeleccionada} value={scenario.lineaMonto} onChange={e => actualizar('lineaMonto', e.target.value)} className="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-800 dark:text-slate-100 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-amber-400" />
                        {lineaSeleccionada && <span className="mt-1 block text-[10px] text-slate-400 dark:text-slate-500">Base vigente: {formatMXN(Number(lineaSeleccionada.montoEfectivo))} · {lineaSeleccionada.clasificacion ?? 'Variable'}</span>}
                      </label>
                    </div>
                  </div>
                </div>
                ...
                <div className="xl:col-span-2 space-y-3">
                  <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                    <div className="flex items-center justify-between gap-3 mb-3">
                      <div><p className="text-xs font-semibold text-slate-700 dark:text-slate-200">Resultado del escenario</p><p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">{scenarioQ.codigo}</p></div>
                      <span className={`text-[10px] font-semibold px-2 py-1 rounded-full ${estadoMargen(scenarioResult.ingreso, scenarioResult.margen).cls}`}>{estadoMargen(scenarioResult.ingreso, scenarioResult.margen).label}</span>
                    </div>
                    <div className="space-y-2 text-xs">
                      ...
                      {scenarioResult.ahorro > 0 && <div className="flex justify-between gap-4 text-blue-600 dark:text-blue-400"><span>Ahorro planeado</span><span className="tabular-nums">{formatMXN(scenarioResult.ahorro)}</span></div>}
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <ImpactBox label="vs. forecast" value={scenarioResult.diferenciaVsForecast} />
                      <ImpactBox label="vs. plan vigente" value={scenarioResult.diferenciaVsPlan} />
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                    ...
                    <ResponsiveContainer width="100%" height={165}>
                      <BarChart data={comparacion} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                        ...
                        <Bar dataKey="valor" radius={[5, 5, 0, 0]} isAnimationActive={false}>{comparacion.map(item => <Cell key={item.name} fill={item.color} />)}</Bar>
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
            {qs.map(q => {
              const scenarioForCard = scenarioQ?.quincenaId === q.quincenaId ? scenarioResult : null
              return <ForecastCard key={q.quincenaId} q={q} scenario={scenarioForCard} />
            })}
          </div>
        </>
      )}
      ...
      {data.deudasSinCalendario.totalMensual > 0 && (
        <div className="rounded-xl border border-amber-200 dark:border-amber-800/60 bg-amber-50 dark:bg-amber-950/30 px-4 py-3 flex gap-3">
          ...
          <div className="text-xs text-amber-900 dark:text-amber-200">
            ...
            <p className="mt-1 text-amber-800/80 dark:text-amber-300/80">No las sumo automáticamente al forecast: Milo conoce el abono mensual, pero no en qué quincena cae y podría duplicar una línea ya presupuestada. Calendariza el pago o crea su línea de presupuesto para incorporarlo con certeza.</p>
          </div>
        </div>
      )}
...
function ImpactBox({ label, value }: { label: string; value: number }) {
  const cls = value > 0 ? 'text-rose-600 dark:text-rose-400' : value < 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-300'
  return (
    <div className="rounded-lg bg-slate-50 dark:bg-slate-900/60 p-2.5">
      ...
      <p className={`text-sm font-bold tabular-nums ${cls}`}>{value > 0 ? '+' : ''}{formatMXN(value)}</p>
    </div>
  )
}
