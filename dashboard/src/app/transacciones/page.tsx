      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {transacciones.length === 0 ? (
          <div className="text-center py-20 text-slate-400">
            ...
            <p className="text-sm mt-1">Manda un gasto por WhatsApp o importa el Excel para ver datos aquí</p>
          </div>
        ) : (
            ...
            <tbody className="divide-y divide-slate-50">
              {transacciones.map(tx => (
                <tr key={tx.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3.5 text-slate-500">{formatDate(tx.fecha)}</td>
                  <td className="px-4 py-3.5 text-slate-500">{tx.user?.nombre ?? '—'}</td>
